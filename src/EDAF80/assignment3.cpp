...
edaf80::Assignment3::run()
{
	...
	auto const phong_set_uniforms = [&use_normal_mapping,&has_textures,&light_position,&camera_position,&ambient,&diffuse,&specular,&shininess](GLuint program){
		glUniform1i(glGetUniformLocation(program, "has_textures"), has_textures ? 1 : 0);
	...
	// SPHERE
	auto sphere_shape = parametric_shapes::createSphere(1.5f, 40u, 40u);
	if (sphere_shape.vao == 0u) {
		LogError("Failed to retrieve the mesh for the demo sphere");
		return;
	}

	auto diff_id = bonobo::loadTexture2D("C:/Skola/datorgrafik/res/textures/cobblestone_floor_08_diff_2k.jpg", true);
	auto bump_id = bonobo::loadTexture2D("C:/Skola/datorgrafik/res/textures/cobblestone_floor_08_nor_2k.jpg", true);
	auto rough_id = bonobo::loadTexture2D("C:/Skola/datorgrafik/res/textures/cobblestone_floor_08_rough_2k.jpg", true);
	Node demo_sphere;
	demo_sphere.set_geometry(sphere_shape);
	demo_sphere.set_program(&phong_shader, set_uniforms);

	demo_sphere.add_texture("diff_map", diff_id, GL_TEXTURE_2D);
	demo_sphere.add_texture("bump_map", bump_id, GL_TEXTURE_2D);
	demo_sphere.add_texture("rough_map", rough_id, GL_TEXTURE_2D);
