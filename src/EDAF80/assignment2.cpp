...
edaf80::Assignment2::run()
{
	...
	auto const moving_sphere = parametric_shapes::createSphere(0.5f, 10u, 10u);
	Node sphere;
	Node sphere2;
	sphere.set_geometry(moving_sphere);
	sphere.set_program(&normal_shader, set_uniforms);
	sphere2.set_geometry(moving_sphere);
	...
	for (std::size_t i = 0; i < control_point_locations.size(); ++i) {
		...
		control_point.set_program(&normal_shader, set_uniforms);
		...
		if (interpolate) {
			...
			if (use_linear) {
				...
				//!       using the linear interpolation.
					sphere.get_transform().SetTranslate(interpolation::evalLERP(control_point_locations[i% control_point_locations.size()],
																				control_point_locations[(i+1)% control_point_locations.size()],
																				path_pos-i));
				...
				sphere.get_transform().SetTranslate(interpolation::evalCatmullRom(control_point_locations[i % control_point_locations.size()],
																				  control_point_locations[(i + 1) % control_point_locations.size()],
																				  control_point_locations[(i + 2) % control_point_locations.size()],
																				  control_point_locations[(i + 3) % control_point_locations.size()],
																				  catmull_rom_tension,
																				  path_pos - i));
				sphere2.get_transform().SetTranslate(interpolation::evalCatmullRom(control_point_locations[i % control_point_locations.size()],
					control_point_locations[(i + 1) % control_point_locations.size()],
					control_point_locations[(i + 2) % control_point_locations.size()],
					control_point_locations[(i + 3) % control_point_locations.size()],
					catmull_rom_tension*2,
					path_pos - i));
				path_pos += pos_velocity; // step forward
			}
		}
