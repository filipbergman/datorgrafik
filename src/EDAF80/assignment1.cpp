...
int main()
{
	...
	SpinConfiguration const sun_spin{glm::radians(20.0f), glm::two_pi<float>() / 6.0f};
	...
	std::stack <CelestialBody*> stackOfBodies;
	std::stack <glm::mat4> stackOfMatrices;
	...
	auto const ring = parametric_shapes::createCircleRing(1.0f, 0.8f, 80u, 8u);
	...
	saturn.set_ring(ring, &celestial_ring_shader, saturn_ring_texture);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		stackOfBodies.push(&sun);
		stackOfMatrices.push(glm::mat4(1.0f));
		while (!stackOfBodies.empty()) {
			CelestialBody* body = stackOfBodies.top();
			glm::mat4 matrix = stackOfMatrices.top();

			glm::mat4 parentMatrix = body->render(animation_delta_time_us, camera.GetWorldToClipMatrix(), matrix);
			stackOfBodies.pop();
			stackOfMatrices.pop();

			std::vector<CelestialBody*> childVector = body->get_children();
			for (int i = childVector.size() - 1; i >= 0 ; i--) {
				stackOfBodies.push(childVector[i]);
				stackOfMatrices.push(parentMatrix);
			}
		}
		//
