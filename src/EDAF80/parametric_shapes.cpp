                              ...
                              unsigned int const vertical_split_count)
{
	...
	glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(glm::vec3)
		/*12 floats in "vertices", each 32 bits(4 bytes)*/,
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, /* how many bytes should the buffer contain? */index_sets.size() * sizeof(glm::uvec3)
		,
	             /* where is the data stored on the CPU? */index_sets.data(),
	             /* inform OpenGL that the data is modified once, but used often */GL_STATIC_DRAW);

	data.indices_nb = /* how many indices do we have? */index_sets.size() * 3;
                                ...
                                unsigned int const latitude_split_count)
{
	...
	for (unsigned int i = 0u; i < longitude_slice_vertices_count; ++i) {
		...
		for (unsigned int j = 0u; j < latitude_slice_vertices_count; ++j) {
			...
			// vertex
			vertices[index] = glm::vec3(distance_to_centre * sin_theta*sin_phi,
				-(distance_to_centre * cos_phi),
				distance_to_centre*cos_theta*sin_phi);

			// texture coordinates, x låg tidigare på j och latitude, vilket är fel då latitude är i y(jag bytte då plats på dom) 
	...
	for (unsigned int i = 0u; i < longitude_slice_edges_count; ++i) {
		for (unsigned int j = 0u; j < latitude_slice_edges_count; ++j) {
			...
			index_sets[index] = glm::uvec3(latitude_slice_vertices_count * (i + 0u) + (j + 0u),
				...
				latitude_slice_vertices_count * (i + 1u) + (j + 1u));
			++index;
			// This solution makes the normals point in the wrong direction
			/*
			index_sets[index] = glm::uvec3(latitude_slice_vertices_count * (i + 0u) + (j + 0u),
				latitude_slice_vertices_count * (i + 0u) + (j + 1u),
				latitude_slice_vertices_count * (i + 1u) + (j + 1u));
